...
void GWYMakerClass::StartSavingFromGallery(UnicodeString GwyFileAdd, int ImageCounts)
{
...
//			mainForm->LogToFile("Caption = " + mainForm->myImagerThread->Images[mainForm->myImagerThread->images_index[i]].ImageName + " , Sum = " + IntToStr(Sum));
		}
		else {
			Sum = Sum + mainForm->GalFrm->image_address_array[i]->LabelName->Caption.Length();
//			mainForm->LogToFile("Caption = " + mainForm->GalFrm->image_address_array[i]->LabelName->Caption + " , Sum = " + IntToStr(Sum));
		}
	}
	int DataSize = ((211 +  (Resolution * Resolution * 8) + Pallet.Length()) *  ImageCount) + Sum;
//	mainForm->LogToFile("DataSize = " + IntToStr(DataSize));
