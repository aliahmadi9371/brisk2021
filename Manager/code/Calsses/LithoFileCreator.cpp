...
{
	mainForm->LogToFile("LithoFileCreator::CreateXfile");
	double a1,c1;
	double a2,b2;
	double a3,b3,c3;
...
//		CreateDirectoryA(dir.c_str(),NULL);
//	}
//	AnsiString name = UploadPath + "/litho/Xpoints.txt";
//	DeleteFileW(name);
//	ofstream *file;
//	file = new ofstream(name.c_str(), ios::out);
//	AnsiString tmp;
	for(int i=0; i < 2 * p + (4 * o) - 1; i++){
		LithoXFile[i] = (int)(points[i+1] - points[i]);
...
{
	mainForm->LogToFile("LithoFileCreator::CreateYfile");
	double a1,c1;
	double a2,b2;
	double a3,b3,c3;
...
//		CreateDirectoryA(dir.c_str(),NULL);
//	}
//	AnsiString name = UploadPath + "/litho/Ypoints.txt";
//	DeleteFileW(name);
//	ofstream *file;
//	file = new ofstream(name.c_str(), ios::out);
//	AnsiString tmp;
	for(int i=0; i < 2 * p + (4 * o) - 1; i++){
        LithoYFile[i] = (int)(points[i+1] - points[i]);
//		tmp = IntToStr((int)(points[i+1] - points[i]))+'\n';
//		file->write(tmp.c_str(), tmp.Length());
	}
//	file->close();
...
//{
//   mainForm->LogToFile("LithoFileCreator::CreateVfile");
//	if( !DirectoryExists(UploadPath) ){
//		AnsiString dir = UploadPath;
//		CreateDirectoryA(dir.c_str(),NULL);
//	}
